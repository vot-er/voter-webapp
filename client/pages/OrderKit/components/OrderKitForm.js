...
export class OrderForm extends React.Component {
  ...
  render() {
    const { addressLine1, addressLine2, zipcode, city, state } =
      this.props.form;
      ...
      <div>
        <form onSubmit={this.handleNativeSubmit.bind(this)}>
          ...
          <input
            ...
            name="addressLine2"
            value={addressLine2}
            placeholder="Apartment/suite number, unit etc. (optional) "
          />
          <div className="form__row">
            <div
              className="form__row__col form__row__col--left"
            ...
            <div
              className="form__row__col form__row__col--right"
              style={{ paddingLeft: 8 }}
            >
              <label className="form__label">State</label>
              <Select
                options={this.getStateOptions()}
                onChange={(e) => this.onStateSelectChange(e)}
                className="form__control"
                name="state"
                value={this.getStateOptionFromValue(state)}
                id="state-select"
