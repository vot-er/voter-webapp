...
export class PasswordResetPage extends React.Component {
  ...
  componentDidMount() {
    const { user: userId, token: resetToken } = querystring.parse(
  ...
  async verifyResetToken(userId, resetToken) {
    try {
      if (!userId || !resetToken)
        return this.setState({ tokenIsAuthorized: false });
  ...
  changePassword = async (newPassword, newPasswordRepeat) => {
    try {
      ...
      await this.props.authActions.resetPassword(
        userId,
        resetToken,
        newPassword
      );
  ...
  renderUnauthorized() {
    return (
      <div>
        ...
        <Link to="/password/request">Please try again.</Link>
      </div>
    );
  }
