...
export class BulkUpdatePage extends React.Component {
  ...
  renderResults() {
    ...
    return (
      <table>
        ...
        <tbody>
          {results.map((result, r) => this.renderResultRow(result, r))}
        </tbody>
      </table>
    );
  }
  ...
  async onFormSubmit(e) {
    ...
    try {
      ...
      const { results } = await this.props.bulkAssign({
        codes: this.getCodes(),
      });
      this.setState({
        results,
      ...
      <div className="fill fill-height flex-column bulk-update-page">
        ...
        <form onSubmit={this.onFormSubmit.bind(this)}>
          <textarea
            ...
            placeholder="Add a list of line-separated codes to attach to kits without codes and marked for fulfillment."
          />
          <div>
            <SubmitButton
...
export default withRouter(connect(null, mapDispatchToProps)(BulkUpdatePage));
