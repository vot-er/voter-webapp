...
import {getOne as getUser, updateOrganization as updateUserOrganization} from '../../actions/userActions';
...
export class UserShowAdminPage extends React.Component {
  ...
  render() {
    const {
      user, isSubmitting, organizationId
    } = this.state;
    if (!user) {
      return <div>
    ...
    return (
      <div className="fill">
        ...
        <div className="fill task-page">
          ...
          <div>Reassign to organization:
            <form onSubmit={this.submitAssignOrganization.bind(this)}>
              <input className="form__control" placeholder="Put organization id here." value={organizationId} onChange={e => this.setState({organizationId: e.target.value})} />
  ...
  async submitAssignOrganization(e) {
    ...
    try {
      ...
      await this.props.updateUserOrganization(match.params.userId, organizationId);
      await this.getUser();
      this.setState({
        isSubmitting: false,
      });
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    updateUserOrganization: bindActionCreators(updateUserOrganization, dispatch),
  };
}
